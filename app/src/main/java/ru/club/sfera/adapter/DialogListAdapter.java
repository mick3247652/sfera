...
import ru.club.sfera.model.Chat;
...
public class DialogListAdapter extends BaseAdapter implements Constants {
	...
	static class ViewHolder {
        ...
        public CircularImageView chatOpponentAuthorIcon;
        public ImageView chatOpponentAuthorOnlineIcon;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
            ...
            viewHolder.chatOpponentAuthorIcon = (CircularImageView) convertView.findViewById(R.id.chatOpponentAuthorIcon);
            viewHolder.chatOpponentAuthorOnlineIcon = (ImageView) convertView.findViewById(R.id.chatOpponentAuthorOnlineIcon);
        ...
        viewHolder.chatOpponentAuthorOnlineIcon.setTag(position);
        viewHolder.chatOpponentAuthorIcon.setTag(position);
        ...
        if (chat.getWithUserVerified() == 1) {

            viewHolder.chatOpponentAuthorIcon.setVisibility(View.VISIBLE);

        } else {

            viewHolder.chatOpponentAuthorIcon.setVisibility(View.GONE);
        }

        if (chat.getWithUserOnline() && chat.getWithUserAllowShowOnline() == ENABLED) {

            viewHolder.chatOpponentAuthorOnlineIcon.setVisibility(View.VISIBLE);

        } else {

            viewHolder.chatOpponentAuthorOnlineIcon.setVisibility(View.GONE);
        }
